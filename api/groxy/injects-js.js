 ...
 try {
  ...
  `;
   ...
   void async function() {
     if (document.querySelector('[id="injectcss"]')) {
       return;
     }
 ...
 setInterval(async function() {
   ...
   try {
     if (!document.querySelector('.Hero-blurb>h1').innerText.includes('Go Bananas')) {
       document.querySelector('.Hero-blurb>h1').innerText = 'Go Bananas';
       document.querySelector('.Hero-blurb>h1').style.visibility = 'visible';
     }
   } catch (e) {}
 ...
 async function transformLinks(attr) {
   ...
   for (let i = 0; i < hostTargetList_length; i++) {
     ...
     for (let x = 0; x < pkgs_length; x++) {
       ...
       let hash = '';
       if (pkgs[x][attr].includes('#')) {
         hash = '#' + pkgs[x][attr].split('#')[1];
       }
       let char = '?';
       if (pkgs[x][attr].includes('?')) {
         char = '&';
 ...
 if (!globalThis.backupElements) {
   globalThis.backupElements = {};
 }
 async function backupNode(element) {
   try {
     ...
     backup.promise = new Promise((resolve, reject) => {
       globalThis.backupElements['' + backupId] = {
         "promise": backup.promise,
         "resolve": resolve
       };
     });
     backup.onerror = function(e) {
       globalThis.backupElements[backupId].resolve();
     }
     backup.onload = function(e) {
       globalThis.backupElements[backupId].resolve();
     }
 ...
 function addUserAgent() {
   ...
   if (h) {
     h.setAttribute('user-agent', navigator.userAgent);
   }
